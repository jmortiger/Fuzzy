...
class WPostSearchResultsRedux extends StatefulWidget {
  // #region Logger
  // ignore: unnecessary_late
  static late final lRecord = lm.genLogger("WPostSearchResultsRedux");
  static lm.Printer get print => lRecord.print;
  static lm.FileLogger get logger => lRecord.logger;
  // #endregion Logger
  final Iterable<ValueAsync<E6PostResponse>> posts;
  ...
  @override
  State<WPostSearchResultsRedux> createState() =>
      _WPostSearchResultsReduxState();
  ...
  static Widget directResults(List<int> postIds) => FutureBuilder(
        future: (E621
            ...
            .then((v) => (dc.jsonDecode(v.responseBody)["posts"] as List).map(
                  (e) => ValueAsync<E6PostResponse>(
                    value: E6PostResponse.fromJson(e),
                  ),
                ))),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            try {
              return WPostSearchResultsRedux(
                posts: snapshot.data!,
...
class _WPostSearchResultsReduxState extends State<WPostSearchResultsRedux> {
  ...
  GridView _makeGridView(Iterable<ValueAsync<E6PostResponse>> posts) {
    return widget.useLazyBuilding
        ? GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // }
              var data = posts.elementAtOrNull(index)?.$; //posts.tryGet(index);
        ...
        : GridView.count(
            ...
            children: (Iterable<int>.generate(estimatedCount)).reduceUntilTrue(
                ...
                (accumulator, _, index, __) =>
                    posts.elementAtOrNull(index)?.$ != null
                        ? (
                            accumulator
                              ..add(
                                // constructImageResult(posts.tryGet(index)!, index),
                                constructImageResult(
                                    posts.elementAtOrNull(index)!.$, index),
                              ),
                            false
                          )
                        : (accumulator, true),
  ...
  WImageResult constructImageResult(E6PostResponse data, int index) =>
      WImageResult(
        ...
        index: index,
        postsCache:
            widget.disallowSelections ? widget.posts.map((e) => e.$) : null,
